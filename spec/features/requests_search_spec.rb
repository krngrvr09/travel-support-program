...
feature "Requests Search", "" do
	fixtures :all
	scenario "Search by full name", :js => true do
	...
	scenario "Search by nickname", :js => true do
		...
		visit travel_sponsorships_path
		# Use the event filter
		fill_in 'q_user_nickname_or_event_name_or_user_profile_full_name_cont', :with => users(:luke).nickname.upcase
		click_button "search"
		# # Check the url to ensure that the form have been submitted
		current_url.should match /event_id_in/
		# If so, the request should be in the first page
	...
	scenario "Search by event name", :js => true do
		...
		visit travel_sponsorships_path
		# Use the event filter
		fill_in 'q_user_nickname_or_event_name_or_user_profile_full_name_cont', :with => events(:dagobah_camp).name.split(" ").first.upcase
		click_button "search"
		# # Check the url to ensure that the form have been submitted
		current_url.should match /event_id_in/
		# If so, the request should be in the first page
